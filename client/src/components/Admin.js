    ...
    return ( <
        ...
        h2 > Blog Posts < /h2> {
            blogs.map((getBlog, index) => ( <
                div className = "view"
                key = { index } > { getBlog.id } <
                h2 > Title: { getBlog.title } < /h2>     <
                p > Decription: { getBlog.description } < /p>     <
                DeleteBlogs id = { getBlog._id }
                afterDelete = { BlogAfterDelete }
                /> <
                /div>
            ))
        } <
        ...
        h2 > New Customer < /h2> {
            customer.map((getCustomer, index) => ( <
                ...
                DeleteCustomer id = { getCustomer._id }
                afterDelete = { CustomerAfterDelete }
                /> <
                /div>
            ))
        } <
